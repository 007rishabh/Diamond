...
const AdminShowProduct = () => {
  ...
  return (
    <View>
    {
        products.map((item)=>{
            return (
                <View key={item.id} style={{height:"auto",backgroundColor:'#a29bfe',marginTop:20,borderRadius:8,padding:20}}>
                <View style={{flexDirection:'row'}}>
                <Text style={{fontSize:20}}>
                {item.name}
                </Text>
                <TouchableOpacity style={{marginLeft:'auto'}} onPress={deleteProduct}>
